...
public abstract class LazyFacilityBag<T> implements LazyBag<T> {
	...
	private Collection<T> installEntities() {
		...
		try {
			...
			if (value instanceof LazyFacilityBag) {
				final LazyFacilityBag lazy = (LazyFacilityBag) value;
			...
			throw new OrmException("Unable to query lazy facilityMetadata collection with dataContainer [" + dataContainer + "].", e);
		}
	}
