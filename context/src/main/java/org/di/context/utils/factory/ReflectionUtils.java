 ...
 */
package org.di.context.utils.factory;
...
import org.di.context.excepton.starter.IoCStartException;
...
public class ReflectionUtils {
     ...
     */
    private static Reflections configureReflection(Class<?>[] classes, String[] modulePackages) {
        ...
        final List<URL> urls = new ArrayList<>(ClasspathHelper.forPackage("org.di.context"));
        for (String path : modulePackages) {
     ...
     * @return new Reflections instance
     */
    private static Reflections configureReflection(String[] paths, String[] modulePackages) {
        final ConfigurationBuilder builder = new ConfigurationBuilder();
        builder.filterInputsBy(new FilterBuilder().include(".*\\.class"));
        builder.setScanners(new SubTypesScanner(), new TypeAnnotationsScanner());
        builder.setUrls();
     ...
     * @return new Reflections instance
     */
    public static Reflections configureScanner(String[] modulePackages, Class<?> mainSource) {
        ...
        if (scanPackage != null) {
            ...
            if (packages.length > 0) {
                reflections = configureReflection(packages, modulePackages);
            } else if (classes.length > 0) {
                reflections = configureReflection(classes, modulePackages);
            } else {
                throw new IoCStartException("IoCError - Unavailable create reflections [no find packages or classes to scan].");
            }
        } else {
            reflections = configureReflection(new Class[]{mainSource}, modulePackages);
        }
